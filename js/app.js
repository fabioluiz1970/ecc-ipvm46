...
/* =======================================================
   3. CADASTRO: INCLUIR, ALTERAR E EXCLUIR (LINHA A LINHA)
======================================================== */
...
document.getElementById("cronogramaForm").addEventListener("submit", function(e) {
    ...
    if (idEdit) {
        ...
        if(index !== -1) {
            // Mantém os dados reais já preenchidos
...
function editCronograma(id) {
    ...
    document.getElementById("crono-acao").value = item.acao;
...
/* =======================================================
   7. RENDERIZAÇÃO GERAL (ATUALIZA TUDO)
======================================================== */
function renderAll() {
    // 1. Renderiza Listas do Cadastro
        ...
        params[tipo].forEach((val, i) => {
            divLista.innerHTML += `
                <div class="cad-item">
                    <span>${val}</span>
                    <div>
                        <button class="btn-alterar" onclick="editBase('{i})">Alterar</button>
                        <button class="btn-excluir" onclick="delBase('{i})">Excluir</button>
                    </div>
                </div>`;
        });

        // 2. Atualiza Selects do Cronograma
        let selectId = `crono-${tipo === 'locais' ? 'local' : tipo === 'equipes' ? 'equipe' : tipo === 'atividades' ? 'atividade' : 'acao'}`;
